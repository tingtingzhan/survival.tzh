...
```{r}
(s0 = survfit(os ~ 1, data = rotterdam))
```
...
```{r}
(s1 = survfit(os ~ size, data = rotterdam))
```
...
```{r}
(s2 = survfit(os ~ size + meno, data = rotterdam))
```
...
s1 = subset_survfit(os ~ size, subset = id, data = rotterdam)
...
#| echo: false
list(survfitlist = s) |>
  fastmd::render2html(file = 'survfitlist')
```
...
```{r}
m = msurvfit(c(rfs, os) ~ menopausal, data = rotterdam)
```
